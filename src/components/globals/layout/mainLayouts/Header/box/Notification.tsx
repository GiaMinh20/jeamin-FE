...
const notiItem = ` cursor-pointer hover:!bg-sec hover:!text-white hover:!ml-[10px] transition-all duration-600
flex items-center justify-between text-[#333]  shadow-lg font-[600] my-[8px] p-[9px] border border-[#1f1f671c]`;
...
const NotiItem = ({ item, close }) => {
  ...
  return (
    <div
      className={notiItem}
      style={{ backgroundColor: item.IsRead ? "#fff" : "#f6984c21", borderRadius: '12px 0' }}
      onClick={() => {
        if (!item.IsRead) {
          item.IsRead = true;
